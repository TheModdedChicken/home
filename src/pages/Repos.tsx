...
async function getRepos() {
  ...
  return res.data
    ...
    .sort(
      // Sort by recently updated to never updated
      (a, b) =>
        new Date(b.updated_at || new Date()).getTime() -
        new Date(a.updated_at || new Date()).getTime(),
    )
...
const Repos: Component = () => {
  ...
  return (
    <div>
      ...
      <Switch>
        <Match when={["pending", "refreshing"].includes(repos.state)}>
          <p>Loading repos...</p>
        </Match>
        <Match when={["errored", "unresolved"].includes(repos.state)}>
          <h5>Failed to load repos :/</h5>
        </Match>
        <Match when={repos.state === "ready"}>
          <p>
            <i>
              The repositories listed here aren't necessarily in active
              development!
            </i>
          </p>
